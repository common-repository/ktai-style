...
define('KTAI_AUTH_COOKIE', 'wordpress_mobile_' . COOKIEHASH);
define('KTAI_SECURE_AUTH_COOKIE', 'wordpress_mobile_sec_' . COOKIEHASH);
define('KTAI_COOKIE', 'wordpress_mobile_logged_in_' . COOKIEHASH); // obsolute
...
public function get_sid() {
	...
	if ($Ktai_Style->ktai->get('cookie_available')) {
		$cookie_name = is_ssl() ? KTAI_SECURE_AUTH_COOKIE : KTAI_AUTH_COOKIE;
 ...
 * @since	2.0.0
 */
public function set_cookie($user_id, $sid, $expiration, $remember, $secure = '') {
	if ( '' === $secure ) {
		$secure = is_ssl() ? true : false;
	}
	if ( $secure ) {
		$auth_cookie_name = KTAI_SECURE_AUTH_COOKIE;
		$scheme = 'secure_auth';
	} else {
		$auth_cookie_name = KTAI_AUTH_COOKIE;
		$scheme = 'auth';
	}
	$expire = $remember ? $expiration : 0;
	$logged_in_cookie = wp_generate_auth_cookie($user_id, $expiration, 'logged_in');
	do_action('set_logged_in_cookie', $logged_in_cookie, $expire, $expiration, $user_id, 'logged_in');

	// Set httponly if the php version is >= 5.2.0
	if ( version_compare(phpversion(), '5.2.0', 'ge') ) {
		setcookie($auth_cookie_name, $sid, $expire, PLUGINS_COOKIE_PATH, COOKIE_DOMAIN, $secure, true);
		...
		setcookie($auth_cookie_name, $sid, $expire, PLUGINS_COOKIE_PATH, $cookie_domain, $secure);
		setcookie(LOGGED_IN_COOKIE, $logged_in_cookie, $expire, COOKIEPATH, $cookie_domain);
		if ( COOKIEPATH != SITECOOKIEPATH ) {
			setcookie(LOGGED_IN_COOKIE, $logged_in_cookie, $expire, SITECOOKIEPATH, cookie_domai);
		}
	}
}
...
public function erase_cookie() {
	setcookie(KTAI_AUTH_COOKIE,        '', time() - 31536000, PLUGINS_COOKIE_PATH, COOKIE_DOMAIN);
	setcookie(KTAI_SECURE_AUTH_COOKIE, '', time() - 31536000, PLUGINS_COOKIE_PATH, COOKIE_DOMAIN);
...
public function set_session($user_id, $sid = NULL, $data = NULL, $remember = false) {
	global $wpdb;
	if ($this->base->ktai->get('cookie_available')) {
	...
	$tid_hash = $this->base->ktai->get('term_ID') ? wp_hash($this->base->ktai->get('term_ID')) : '';
	$sub_hash = $this->base->ktai->get('sub_ID')  ? wp_hash($this->base->ktai->get('sub_ID')) : '';
	...
	$this->data       = $data;
	if ( $this->base->ktai->get('cookie_available') ) {
		$this->set_cookie($user_id, $sid, $expires, $remember);
	}
	return $sid;
}
 ...
 * @return	none
 * @since	0.9.3
 * Based on auth_redirect() at wp-includes/pluggable.php of WP 2.9.2
 */
public function auth_redirect() {
	...
	if ( $secure && !is_ssl() && false !== strpos($_SERVER['REQUEST_URI'], KtaiStyle::ADMIN_DIR) ) {
	...
	if ( $user_id = $this->check_session(apply_filters('auth_redirect_scheme', '')) ) {
		do_action('auth_redirect', $user_id);
		
		// If the user wants ssl but the session is not ssl, redirect.
		if ( !$secure && get_user_option('use_ssl', $user_id) && false !== strpos($_SERVER['REQUEST_URI'], KtaiStyle::ADMIN_DIR) ) {
			if ( 0 === strpos($_SERVER['REQUEST_URI'], 'http') ) {
				wp_redirect(preg_replace('|^http://|', 'https://', $_SERVER['REQUEST_URI']));
				exit();
			} else {
		...
		return;  // The session is good so we're done
	}
		
	// The session is no good so force login
	nocache_headers();
	if ( is_ssl() ) {
		$proto = 'https://';
	} else {
	...
	$redirect = $this->base->strip_host($proto . $_SERVER['HTTP_HOST'] . remove_query_arg('ksid'));
	wp_redirect($this->base->get('plugin_url') . KtaiStyle::LOGIN_PAGE . '?error=oldsession&redirect_to=' . urlencode($redirect));
	exit();
}
...
public function nonce_ays($action) {
	...
	$title = $this->base->encode->to_mobile(__('WordPress Confirmation', 'ktai_style'));
	...
	$html .= $this->base->encode->to_mobile($add_html);
