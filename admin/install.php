...
public static function install() {
	...
	dbDelta(apply_filters('ktai_style_create_ktaisession', $sql));
	return;
}
...
public static function uninstall() {
	...
	$wpdb->query(apply_filters('ktai_style_drop_ktaisession', $sql));
	return;
}
 ...
 * @since   1.80
 */
public static function uninstall_sitewidely() {
	$blogs = get_blog_list(0, 'all', false);
	if (is_array($blogs)) {
		reset($blogs);
		foreach((array) $blogs as $key => $details) {
