...
if ( !$Ktai_Style->get('encoding_converted') ) {
	$encoding = $Ktai_Style->detect_encoding();
	$comment_author  = $Ktai_Style->decode_from_ktai($comment_author, $encoding);
	$comment_content = $Ktai_Style->decode_from_ktai($comment_content, $encoding);
}
if ( !$Ktai_Style->encode->check($_POST['email'], 'ASCII') || !$Ktai_Style->encode->check($_POST['url'], 'ASCII') ) {
