...
* Available phone/gadgets: NTT docomo, au by KDDI, SoftBank, E-MOBILE, WILLCOM, iPhone/iPod touch (need to configure), Android (need to configure), BlackBerry (need to configure),  Nokia, Motolora, SAMSUNG, Windows Mobile terminals, Palm OS handheld, Opera Mini/Opera Mobi browsers, PSP, Nintendo DS, Sharp Zaurus.
...
= 2.1.0 (2011-09-xx) =
* Enabled SSL support for mobile admin feature.
* Can specify pc/mobile only content block by `[pc-only]...[/pc-only]`, `[mobile-only]...[/mobile-only]` format.
* Can specify mobile site link as `ktai` attribute of a elements. For example: `<a href="http://pc.example.com/" ktai="http://mobile.example.com/">Example Site</a>`
* Convert a URL or embeded object of YouTube into the link to mobile YouTube. [Ticket #12309](http://sourceforge.jp/ticket/browse.php?group_id=2362&amp;tid=12309)
* Treat BlackBerry as touch phones (same as iPhone, Android). Need to configure to apply for BlackBerry.
* Treat Motorola phones as generic phone, except V702MO for Vodafone Japan.
* Change the name of languages resource directory from "lang" into "languages".
* Protected/private post title reflects `protected_title_format`/`private_title_format` filter. 
* Split mobile page manually by `<!--KTAI_PAGE-->` tags.
* Arguments of `ks_pages_menu()` are now applyed `wp_page_menu_args` filter. Each title of pages are now applyed `the_title` filter.
* The output of `ks_login_link()` is now reflecting `loginout` filter. `ks_get_login_url()` is now reflecting `login_url` filter. `ks_get_logout_url()` is now reflecting `logout_url` filter.
* Use iconv extension if mbstring extension does not exist.
* Improved compatibility with Advanced PHP Cache.
* Raw URLs in comment text is now a link using redirect feature.
* Do not use a redirect page for mobile search engine.
* Delete all old style `XXXXXX/ktai_style.php` actions and filters and actions. Use `ktai_XXXXXX` filters or actions.
* Added exclusive filter `ktai_self_url`.
* Delete third parameter `$ext` from `ktai_die_logo` and `ktai_login_logo` filter.
* Use and touch global variable `$allowedposttags, $allowedtags` to filter HTML tags.
* Separate patches of other plugin into phase 1 and 2.
* Disable [WP-FollowMe plugin](http://wordpress.org/extend/plugins/wp-followme/) at mobile output.
...
* When clicking "Logout" at comment form for log-in users, return to the comment form instead of login form.
