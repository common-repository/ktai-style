...
class KtaiStyle_Config {
	...
	const LINK_COLOR_URL_EXAMPLE = 'http://wordpress.org/extend/plugins/ktai-style/';
...
public function add_link($links, $file) {
	if ( $file == $this->base->get('plugin_basename') ) {
...
public function add_menu() {
	$hookname = add_menu_page(__('Ktai Style Configuration', 'ktai_style'), __('Mobile Output', 'ktai_style'), 'manage_options', self::OPTIONS, array($this, 'misc_page'), $this->base->get('plugin_url') . KtaiStyle::CONFIG_DIR . '/icon16.png');
	add_action('admin_print_styles-' . $hookname, array($this, 'icon_style'));
	
	$hookname = add_submenu_page(self::OPTIONS, __('Ktai Style Configuration', 'ktai_style'), __('Behavior', 'ktai_style'), 'manage_options', self::OPTIONS, array($this, 'misc_page'));
	add_action('admin_print_styles-' . $hookname, array($this, 'icon_style'));

	$hookname = add_submenu_page(self::OPTIONS, __('Theme for mobile', 'ktai_style'), __('Theme', 'ktai_style'), 'switch_themes', self::THEME_OPTIONS, array($this, 'theme_page'));
	add_action('admin_print_styles-' . $hookname, array($this, 'icon_style'));
	add_action('load-' . $hookname, array($this, 'theme_page_header'));
	add_action('load-' . $hookname, array($this, 'update_common_theme'));

	$hookname = add_submenu_page(self::OPTIONS, __('Mobile Theme Configuration', 'ktai_style'), __('Color &amp; Format', 'ktai_style'), 'manage_options', self::COLOR_OPTIONS, array($this, 'color_page'));
	add_action('admin_print_styles-' . $hookname, array($this, 'icon_style'));

	$hookname = add_submenu_page(self::OPTIONS, __('Delete Configuration', 'ktai_style'), __('Delete Options', 'ktai_style'), 'manage_options', self::DELETE_OPTIONS, array($this, 'delete_page'));
	add_action('admin_print_styles-' . $hookname, array($this, 'icon_style'));
}
...
?>
<link rel="stylesheet" type="text/css" id="ktai-style" href="<?php echo $this->base->get('plugin_url') . KtaiStyle::CONFIG_DIR; ?>/config.css" />
<?php
}
...
<?php
	...
	$allow_pictograms  = $this->set_radio_button($this->base->get_option('ks_allow_pictograms'));
	$separate_comments = $this->set_radio_button($this->base->get_option('ks_separate_comments'));
	$require_term_id   = $this->set_radio_button($this->base->get_option('ks_require_term_id'));
	$treat_as_internal = $this->base->get_option('ks_treat_as_internal');
?>
<div class="wrap">
...
<?php
	...
	$theme         = $this->base->get_option('ks_theme');
	$theme_touch   = $this->base->get_option('ks_theme_touch');
	$theme_mova    = $this->base->get_option('ks_theme_mova');
	$theme_foma    = $this->base->get_option('ks_theme_foma');
	$theme_ezweb   = $this->base->get_option('ks_theme_ezweb');
	$theme_sb_pdc  = $this->base->get_option('ks_theme_sb_pdc');
	$theme_sb_3g   = $this->base->get_option('ks_theme_sb_3g');
	$theme_willcom = $this->base->get_option('ks_theme_willcom');
	$theme_emobile = $this->base->get_option('ks_theme_emobile');
...
<?php if ( !empty($theme_name) ) :
	...
	$preview_link = htmlspecialchars( add_query_arg( array(
		...
		'_wpnonce' => $this->base->create_anon_nonce('switch-theme_' . $theme_name),
		'TB_iframe' => 'true',
	), $preview_link ) );
...
<?php
	...
	$date_color          = $this->base->get_option('ks_date_color');
	$author_color        = $this->base->get_option('ks_author_color');
	$comment_type_color  = $this->base->get_option('ks_comment_type_color');
	$external_link_color = $this->base->get_option('ks_external_link_color');
	$edit_color          = $this->base->get_option('ks_edit_color');
	$year_format         = $this->base->get_option('ks_year_format');
	$month_date_format   = $this->base->get_option('ks_month_date_format');
	$time_format         = $this->base->get_option('ks_time_format');
?>
<div class="wrap">
<div id="icon-ktaistyle" class="icon32"><br /></div>
...
<td id="external_link_color_sample" style="color:<?php echo esc_attr($external_link_color); ?>;"><?php _e(self::LINK_COLOR_URL_EXAMPLE, 'ktai_style'); ?></td>
</tr><tr>
