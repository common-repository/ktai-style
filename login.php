...
private function login_header($title = 'Log in', $message = '', $wp_error = '') {
	...
	$logo_file = $this->base->strip_host($this->base->ktai->get('plugin_url')) . KtaiStyle::INCLUDES_DIR . '/wplogo.gif';
	$this->mime_type    = 'text/html';
	$this->iana_charset = $this->base->encode->iana_charset();
	...
	case 'Unknown':
		...
		$this->wrap_start = '<div style="width:19em;margin:0 auto;text-align:left;">';
		$this->wrap_end = '</div>';
...
</head><body bgcolor="#fbfbfb" text="#333333" link="#777777" vlink="#777777"<?php echo $style_body; ?>>
<?php
	echo $this->wrap_start;
	$logo_html = sprintf('<center><h1><img src="%s" alt="WordPress"/></h1></center>', $logo_file);
	$logo_html = apply_filters('ktai_login_logo', $logo_html, $logo_file);
	echo $logo_html;
?><br />
	<?php
	...
	if ($wp_error && $wp_error->get_error_code()) {
		...
		if (! empty($errors)) {
			echo '<p><font color="red">' . apply_filters('login_errors', $errors) . '</font></p>';
		}
		if (! empty($messages)) {
			echo '<p><font color="#ff5566">' . apply_filters('login_messages', $messages) . '</font></p>';
		}
	}
}
...
<?php
	...
	$buffer = $this->base->encode->to_mobile($buffer);
...
private function login() {
	...
	if ( isset($_REQUEST['redirect_to']) ) {
		...
		if ( $secure_cookie && false !== strpos($redirect_to, KtaiStyle::ADMIN_DIR) ) {
	...
	if ( !$secure_cookie && is_ssl() && force_ssl_login() && !force_ssl_admin() && ( false !== strpos($redirect_to, 'https') ) && ( false === strpos($redirect_to, 'http') ) ) {
		$secure_cookie = false;
	}
	
	$user = $this->base->admin->signon('', $secure_cookie);
...
<?php 
	if ($this->base->ktai->get('pcview_enabled')) { 
