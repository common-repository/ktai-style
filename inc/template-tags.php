 ...
 * @return	string  $output
 */
function _ks_internal_link($link, $accesskey, $anchor, $post_password = NULL, $before = '', $after = '') {
	if ($post_password && ! ks_post_password_required()) {
		...
		. $before 
		. '<label' . ks_accesskey_html($accesskey) . '><input type="submit" name="submit" value="' . esc_attr($anchor) . '" /></label>' 
		. $after . '</form>';
	} else {
		$output = $before . '<a href="' . esc_attr($link) . '"' . ks_accesskey_html($accesskey) . '>' . $anchor . '</a>' . $after;
	}
	return $output;
}
...
function ks_login_link() {
	...
	if ( isset($Ktai_Style->admin) ) {
		...
		$output = apply_filters('loginout', $output);
		if ($r['echo']) {
			echo $output;
		}
	}
...
function ks_get_login_url($echo = false, $redirect = '') {
	...
	if ( isset($Ktai_Style->admin) ) {
		...
		$url = apply_filters('login_url', $url, $redirect);
...
function ks_get_logout_url($echo = false, $redirect = '') {
	...
	if ( isset($Ktai_Style->admin) ) {
		...
		$url = apply_filters('logout_url', $url, $redirect);
	} else {
...
function ks_fix_encoding_form($echo = true) {
	...
	. __('Encoding discriminant strings to avoid charset mis-understanding. Symbols:(1)~*', 'ktai_style') 
	. '" />';
	if ($echo) {
		echo $output;
	}
...
function ks_detect_encoding() {
	global $Ktai_Style;
	return $Ktai_Style->detect_encoding();
}
...
function ks_convert_kana($buffer) {
	global $Ktai_Style;
	return $Ktai_Style->encode->halfwidth_kana($buffer);
}
...
function ks_require_term_id_form($action, $method = 'post') {
	...
	if ( !ks_is_loggedin() && ks_option('ks_require_term_id') && method_exists($Ktai_Style->ktai, 'request_uid_form') ) {
		$html = $Ktai_Style->ktai->request_uid_form($action, $method);
	} else {
		$html = sprintf('<form method="%s" action="%s">', $method, esc_attr($action));
	}
	echo apply_filters( 'ktai_require_term_id_form', $html, $action, $method );
}
...
//function ks_ordered_link($count, $max = 10, $link, $anchor = NULL, $format, $echo) {
	$defaults = array(
		'count'  => 1,
		'max'    => 10,
		'link'   => NULL,
 ...
 */
function _ks_page_link($i, $accesskey, $anchor, $post_status, $post_password) {
	if ($i == 1) {
		$output = _ks_internal_link(get_permalink(), $accesskey, esc_html($anchor), $post_password);
	} elseif ('' == get_option('permalink_structure') || 'draft' == $post_status) {
		$output = _ks_internal_link(get_permalink() . '&amp;page=' . $i, $accesskey, esc_html($anchor), $post_password);
	} else {
		$page = user_trailingslashit($i, 'single_paged');
		$output = _ks_internal_link(trailingslashit(get_permalink()) . $page, $accesskey, esc_html($anchor), $post_password);
	}
	return $output;
}
 ...
 * @param	string  $before
 * @param	string  $after
 * @param	string  $prev_anchor
 * @param	string  $next_anchor
...
function ks_posts_nav_dropdown() {
	...
	if ($r['echo'] && (! $Ktai_Style->ktai || ! ($size = $Ktai_Style->ktai->get('page_size')) || $size - 300 >= strlen($output))) {
		echo $output;
	}
	return $output;
}
...
function ks_comments_post_url($id = 0) {
	...
	$address = add_query_arg('view', 'co_post', $address);
	if (isset($_GET['replytocom'])) {
		$address = add_query_arg('replytocom', absint($_GET['replytocom']), $address);
	}
	if ( !ks_is_loggedin() && ks_option('ks_require_term_id') && method_exists($Ktai_Style->ktai, 'request_uid_url') ) {
		$address = $Ktai_Style->ktai->request_uid_url($address);
	}
	return $address;
}
...
function ks_pages_menu() {
	...
	unset($r['args']);
	$args = $r;
	unset($args['sep'], $args['before'], $args['after'], $args['all_page_anchor'], $args['echo']);
	...
	foreach ($pages as $p) {
		...
		$menu[] = '<a href="' . KtaiStyle::strip_host(get_page_link($p->ID)) . '"' . $style . '>' . $a_open . esc_attr(apply_filters('the_title', $p->post_title)) . $a_close . '</a>';
	}
	if ($has_children) {
...
function ks_nav_menu() {
	...
	$r['menu_class'] = '';
	$r['items_wrap'] = '%3$s';
	$r['fallback_cb'] = $r['fallback_cb'] ? $r['fallback_cb'] : 'ks_pages_nemu';
	$r['echo'] = false; // not to echo by wp_nav_menus()
	$menu_html= preg_replace(
		array('!^\s*<li [^>]*>!', 
		      '!(</li>|\s+<ul [^>]*>|</li>\s+</ul>\s+</li>)\s*<li [^>]*>!s', 
		      '!(\s*</ul>\s*)?</li>\s*$!'), 
		array($before,$sep, $after), 
		wp_nav_menu($r) );
	if ( $echo ) {
		echo $menu_html;
	} else {
...
function ks_switch_pc_view() {
	...
	$link = remove_query_arg('menu', $_SERVER['REQUEST_URI']);
	$link = add_query_arg(KtaiStyle::PCVIEW_QUERY , 'true', $link);
