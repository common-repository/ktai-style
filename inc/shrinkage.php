...
class KtaiShrinkage {
	...
	const HIDE_IMAGE_CLASS = 'ktai-hide';
	...
	static public $embeds = array(
		'http://www\.youtube\.com/v/(\w+)' => 'http://m.youtube.com/watch?v=$1'
	);
...
public function __construct() {
	...
	$this->hide_image_regex = '/(^| )(' . preg_quote(self::HIDE_IMAGE_CLASS, '/') . ')( |$)/';
	...
	add_filter('wp_list_categories', array($this->base, 'filter_tags'), 90);
	add_filter('wp_list_pages', array($this->base, 'filter_tags'), 90);
	...
	add_filter('ktai_shrink_content', array($this, 'strip_styles_scripts_del'), 20);
	add_filter('ktai_shrink_content', array($this, 'shrink_embed'), 30);
	add_filter('ktai_shrink_content', array($this->base, 'filter_tags'), 40);
	add_filter('ktai_shrink_content', array($this, 'convert_links'), 50);
	add_filter('ktai_shrink_content', array($this, 'convert_images'), 60);
	add_filter('ktai_trim_excerpt', array($this, 'strip_styles_scripts_del'), 20);
	add_filter('ktai_trim_excerpt', array($this->base, 'filter_tags'), 40);
	add_filter('img_caption_shortcode', array($this, 'img_caption'), 20, 3);
	do_action('ktai_setup_shrinkage');
	return;
}
...
public function shrink_title($title) {
	$protected = apply_filters('protected_title_format', __('Protected: %s'));
	$phrase[0] =     str_replace('%s',    '',           $protected);
	$regex[0] = '/^'.str_replace('%s','(.*)',preg_quote($protected, '/')) . '$/';
	$icon[0] =       str_replace('%s',  '$1',           $protected);
	$icon[0] =       str_replace($phrase[0], '<img localsrc="501" alt="' . $phrase[0] . '" />', $icon[0]);
	$private = apply_filters('private_title_format', __('Private: %s'));
	$phrase[1] =     str_replace('%s',    '',           $private);
	$regex[1] = '/^'.str_replace('%s','(.*)',preg_quote($private, '/')) . '$/';
	$icon[1] =       str_replace('%s',  '$1',           $private);
...
public function convert_links($content) {
	     ...
	     $offset += strlen($replace))
	{
		...
		$replace   = $orig;
		$link_attr = array();
		if (preg_match_all('/\b(class|style|ktai)=([\'"])(' . KtaiStyle::QUOTED_STRING_REGEX . ')\\2/s', $attr_str, $attr_list, PREG_SET_ORDER)) {
...
private function image_inside_link($orig, $href, $url, $link_attr, $anchor, $src, $img_attr) {
	...
	if ($path) {
		if (is_file($path) && ($imagesize = @filesize($path)) > 0 ) {
			...
			if ($this->image_inline && $imagesize <= min($this->base->ktai->get('cache_size'), $size_limit)) {
				$link_html = $orig;
			} else {
...
public function rewrite_link($url, $anchor, $attr) {
	if ( isset($attr['ktai']) ) {
		$link_html = $this->link_element($this->base->strip_host($attr['ktai']), $anchor, $attr);
	} elseif ( preg_match($this->self_urls_regex, $url) ) {
	  ...
	  || $this->base->ktai->is_search_engine() ) {
		...
		if ( !$this->base->ktai->get('use_redir') ) {
	...
	if (isset($attr['class']) && preg_match($this->keep_image_regex, $attr['class'])) {
		...
		if ( defined('KTAI_SHRINKAGE_DEBUG') && KTAI_SHRINKAGE_DEBUG && is_ks_error($thumb_path)) {
			...
			if (! $path) { // link to a image of external sites
				$replace .= $this->link_element($src, $alt, $attr);
			} elseif (! is_dir($path) && $size = @filesize($path) && $size <= $this->base->ktai->get('cache_size')) { // link to the image
				$replace .= $this->link_element($src, $alt, $attr);
...
public function get_thumbnail_flickr($img_src, $img_href = '') {
	if ($img_href) {
		$thumb_url = preg_replace('!(/\d+_[0-9a-z]+)(_(sq|t|s|m|z|o|b))?(\.jpg)$!', '$1_m$4', $img_src, 1);
	} else {
		$thumb_url = preg_replace('!(/\d+_[0-9a-z]+)(_(sq|t|s|m|z|o|b))?(\.jpg)$!', '$1_t$4', $img_src, 1);
	}
	return $thumb_url;
}
...
public function strip_styles_scripts_del($content) {
	...
	$content = preg_replace('#<!--(KTAI_PAGE)-->#', '<!--[$1]-->', $content);
 ...
 * @since	2.1.0
 */
public function shrink_embed($content) {
	if (preg_match_all('#<object .*?><embed src="(' . KtaiStyle::QUOTED_STRING_REGEX . ')" .*?</embed>.*?</object>#', $content, $objects, PREG_SET_ORDER)) {
		foreach($objects as $o) {
			$object = $o[0];
			$src    = $o[1];
			foreach ( (array) self::$embeds as $regex => $url) {
				if (preg_match('#^' . $regex . '#', $src, $match)) {
					$url = preg_replace('#^' . $regex . '.*$#', $url, $src);
					$link = apply_filters('ktai_shrink_embed', sprintf('<a href="%1$s" class="'. self::MOBILE_SITE_CLASS . '">%2$s</a>', esc_url($url), esc_url($url)), $url, $object);
					$content = str_replace($object, $link, $content);
					break;
				}
			}
		}
...
public function shrink_content($content) {
	if (! $this->skip_shrink_content) {
		$content = apply_filters('ktai_shrink_content', $content);
	}
	return $content;
}
...
public function trim_excerpt($text) {
	$content = apply_filters('ktai_trim_excerpt', $content);
...
public function split_page($buffer, $page_num) {
	...
	$page_size = $this->base->ktai->get('page_size') - strlen($header . $navi . '<hr /><hr />' . $navi . $footer) - 32; // 32-byte is space for adding tags by force_balance_tags()
	if ($page_size < 256) { // too small
		...
		$footer = '';
		$page_size = $this->base->ktai->get('page_size') - strlen($header . $navi . '<hr /><hr />' . $navi . $footer) - 32;
	}
	...
	for ($count = 0 ; $count < $page_num ; $count++) {
		$fragment = ks_cut_html($buffer, $page_size, $marker, $this->base->ktai->get('charset'));
		$fragment = preg_replace('/(<!--KTAI_PAGE-->).*$/s', '$1', $fragment);
...
private function get_split_page_navi($num, $rest, $post_password) {
	...
	$navi = $this->base->encode->to_mobile($navi);
...
public function trim_images($content) {
	if ($this->base->ktai->get('cache_size') > 0) {
		...
		for ($offset = 0, $replace = 'X'; 
			...
			if (preg_match($this->self_urls_regex, $src)) {
				...
				if ($total_size > $this->base->ktai->get('cache_size')) {
